 ...
 */
module.exports.doUploadSoap = async function uploadToNetsuite(config) {
  if (!client) {
...
/**
 * Download a single file from Netsuite using SuiteTalk (Web Services)
 * @param {Object} config The configuration object
 * @returns {Promise} A promise that will be resolved once file(s) have been downloaded
 */
module.exports.doDownloadSoap = async function downloadFromNetsuite(config) {
  if (!client) {
    client = await createSoapClient(config);
  }
  ...
  for (const file of files) {
    try {
      ...
      folder = `${config.path}/${path.relative(config.base, path.dirname(file))}`;
      folderID = await getNetsuiteFolderID(config, folder);
      let fileID = await searchFile(file, folderID);
      if (fileID > 0) {
        let content = await getFile(fileID);
        if (content) {
          fs.writeFileSync(file, content, { encoding: 'base64' });
        }
      }
    } catch (error) {
      // It's possible that the cached id of the folder is invalid if the folder was deleted
...
async function getNetsuiteFolderID(config, folder, ignoreCache = false) {
  folder = folder
    .replace(/\\/g, '/') // use forward slashes only
...
async function getFile(fileId) {
  let results = await client.get({
    ':record': {
      ...
      'platformCore:type': {
        attributes: {
          'xsi:type': 'platformCore:RecordType',
        },
        $value: 'file',
      },
    },
  ...
  let status = results['readResponse']['status'];
  let record = results['readResponse']['record'];
...
async function searchFile(name, folderId) {
